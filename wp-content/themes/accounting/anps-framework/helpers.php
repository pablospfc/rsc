...
function anps_get_header() {
    ...
    if(get_option('anps_menu_type', '2')=='5') {
        ...
        if(get_option('anps_menu_boxed') == '1') {
            $menu_type_class .= " site-header-style-full-width-boxed";
        }

        if(get_option('anps_menu_opacity', '100') != '100') {
            $menu_type_class .= " site-header-style-full-width-transparent";
        }
    }
    ...
    <?php //topmenu
    if(($anps_menu_type == "2" || $anps_menu_type == "5") && (get_option('topmenu_style') != '3') && (!isset($page_heading_full) || $page_heading_full=="")) : ?>
    ...
    ?>
        <header class="site-header<?php echo esc_attr($anps_header_styles); ?><?php if(get_option('anps_main_menu_selection', '0')=='0' && !$is_vertical) { echo ' site-header-divider'; } ?>"<?php echo $header_style;?>>
            <?php if(get_option('anps_menu_type', '2')=='6') : ?>
            <div class="nav-wrap">
                <div class="container"><?php anps_get_site_header_center(); //new function ?></div>
            </div>
            <?php elseif(get_option('anps_menu_type', '2')!='5') : ?>
            <div class="nav-wrap">
            ...
            <div class="container preheader-wrap">
                ...
                <?php if((is_active_sidebar( 'large-above-menu'))) : ?>
                    <?php
                        $style_2_class = '';
                        if(get_option('anps_large_above_style') == '2') {
                            $style_2_class .= ' style-2';
                        }
                    ?>

                    <div class="large-above-menu<?php echo esc_attr($style_2_class); ?>"><?php do_shortcode(dynamic_sidebar( 'large-above-menu' ));?></div>
                <?php endif;?>
            ...
            if(is_front_page()==false && anps_get_option($anps_options_data, 'disable_heading')!="0") :
                ...
                if(is_search()) {
                    if(anps_get_option($anps_media_data,'search_heading_bg')) {
                        $style = ' style="background-image: url('.esc_url(anps_get_option($anps_media_data, 'search_heading_bg')).');"';
                    } else {
                        $class = "style-2";
                    }
                } else if( is_404() ) {
                ...
                if(get_option('anps_menu_boxed') == '1' && get_option('anps_menu_type') == '5') {
                    $class .= ' page-heading-top';
                }
                ?>
...
function anps_get_site_header_center() {
    ...
    <div class="nav-bar-wrapper nav-bar-wrapper-right">
        ...
        <?php if( get_option('search_icon', '1') == '1' || get_option('search_icon_mobile', '1') == '1' ): ?>
            ...
            ?>
            <button class="fa fa-search site-search-toggle<?php echo esc_attr($search_class); ?>"><span class="sr-only"><?php esc_html_e('Search', 'accounting'); ?></span></button>
        <?php endif;?>
        <?php
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
...
function anps_get_logo() {
    ...
    elseif (isset($anps_logo) && $anps_logo != "") : ?>
        <?php if(function_exists('pll_register_string')): ?>
            <img
                class="logo-default"
                style="width: <?php echo esc_attr($logo_width); ?>; height: <?php echo esc_attr($logo_height); ?>px"
                alt="Site logo"
                src="<?php pll_e(get_option('anps_logo', '')); ?>"
            >
        <?php else: ?>
    ...
    <?php else: ?>
        <img class="logo-default" style="width: <?php echo esc_attr($logo_width); ?>; height: <?php echo esc_attr($logo_height); ?>px" alt="Site logo" src="http://astudio.si/accounting/wp-content/uploads/2015/04/logo-primary.png">
    <?php endif;
    endif;
    echo '</a>';
}
    ...
    if (isset($anps_sticky_logo) && $anps_sticky_logo != "") : ?>
        <?php if(function_exists('pll_register_string')): ?>
            <img
                class="logo-sticky"
                alt="Site logo"
                src="<?php pll_e($anps_sticky_logo, ''); ?>"
            >
        <?php else: ?>
...
function anps_menu_top() {
    if(get_option('anps_menu_top') == '1' &&
      (get_option('anps_menu_type', '2') == '2' ||
       get_option('anps_menu_type', '2') == '5') &&
       is_front_page()
    ) {
        return ' site-header-absolute';
    }
    ...
    <?php endif;?>
      ...
      /* Max mega menu */
        ...
        if (class_exists('Mega_Menu')) {
            $menu_class = 'site-navigation-max';
        }
        /* END Max mega menu */
        if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
        ...
        <?php if(get_option('anps_menu_social', '') != '' && get_option('anps_menu_social', '') != ';;'): ?>
        <ul class="social-nav">
            <?php
                $socials = explode('|', get_option('anps_menu_social', ''));
            ?>
            <?php foreach($socials as $social): ?>
                ...
                ?>
            <li>
                <a<?php echo $title_attribute; ?> href="<?php echo esc_url($social_url); ?>" target="_blank"<?php if(isset($instance['icon_color'])&&$instance['icon_color']!=''):?> style="color: <?php echo esc_attr($instance['icon_color']);?>"<?php endif; ?>>
                <i class="fa <?php echo esc_attr($social_icon); ?>" aria-hidden="true"></i>
                <span class="sr-only"><?php echo esc_html($social_title); ?></span>
                </a>
            </li>
            <?php endforeach; ?>
        </ul>
        ...
        <?php if (get_option('anps_menu_button') != '' && get_option('anps_menu_button_text', '') != '' ): ?>
            <a class="btn btn-sm" href="<?php echo get_option('anps_menu_button_url', '#'); ?>"><?php echo get_option('anps_menu_button_text', ''); ?></a>
        <?php endif; ?>
    ...
    function anps_setup() {
        ...
        if(get_option('anps_menu_type', 2) == 6) {
            register_nav_menus(array(
                'primary' => esc_html__('Left Navigation', 'accounting'),
                'anps_right' => esc_html__('Right Navigation', 'accounting'),
            ));
        } else {
            register_nav_menus(array(
                'primary' => esc_html__('Primary Navigation', 'accounting'),
            ));
        }
...
function anps_custom_styles($colors = null) {
    global $anps_options_data;
    ...
    $main_footer_divider = get_option('anps_main_footer_divider', '0');
    ...
    $menu_height = get_option('anps_menu_height', '');
    ...
    if( is_front_page() && get_option('anps_front_topbar_bg_color', '') != '' && get_option('anps_menu_type', '2') == 1 ) {
    ...
    $anps_menu_opacity = '100';
    if(is_front_page()) {
        $anps_menu_opacity = get_option('anps_menu_opacity', '100');
    }
    ...
    /* Menu text style */
    $menu_text_style = get_option('anps_menu_text_style', 'uppercase');
?>

::selection { background-color: <?php echo esc_attr($primary_color); ?>; color: #fff; }

.site-navigation {
    text-transform: <?php echo esc_attr($menu_text_style); ?>;
}
...
.social-nav a,
.home .site-header-sticky-active .site-navigation .menu-item-depth-0:not(.current_page_item) > a,
...
.site-header-style-normal .nav-wrap::before {
  ...
  opacity: <?php echo esc_attr($anps_menu_opacity/100); ?>
}

<?php if($anps_menu_opacity != 100): ?>
.site-header-sticky-active.site-header-style-normal .nav-wrap {
    background: none;
}
<?php endif; ?>
...
.site-navigation a:not(.btn):hover, .site-navigation a:not(.btn):focus,
...
@media(min-width: 992px) {
  .site-header-style-full-width .header-wrap::after,
  .site-header-style-full-width .nav-bar-wrapper::after {
    background-color: <?php echo esc_attr($anps_front_bg_color); ?>;
    opacity: <?php echo esc_attr($anps_menu_opacity/100); ?>
  }
...
@media(min-width: 992px) {
  .home .woo-header-cart .cart-contents > i,
...
.site-footer.style-4 .widget_anpstext .important {
...
.alert .close,
.post header {
   font-family: <?php if($font_2=="titillium"){echo "titillium-regular";} else {echo anps_wrap_font(esc_attr($font_2));}?>;
...
.shipping-calculator-button {
  font-family: <?php if($font_1=="titillium"){echo "titillium-bold";} else {echo anps_wrap_font(esc_attr($font_1));}?>;
...
.tp-arr-titleholder
 {
    font-family: <?php if($font_3=="titillium"){echo "titillium-bold";} else {echo anps_wrap_font(esc_attr($font_3));}?>;
...
.comment-list .comment header
 {
  font-family: <?php if($font_1=="titillium"){echo "titillium-regular";} else {echo anps_wrap_font(esc_attr($font_1));}?>;
  <?php if($font_1 === 'Montserrat'): ?>
        font-weight: 500;
   <?php endif; ?>
}
...
.site-footer.style-2,
.site-footer:not(.style-2) > .container {
    border-top: <?php echo esc_attr($main_footer_divider); ?>px solid <?php echo esc_attr($footer_divider_color); ?>;
}
...
@media (min-width: 992px) {
    ...
    .site-header-style-normal .nav-wrap {
        height: <?php echo esc_attr($menu_height); ?>px;
    }
}
...
function anps_custom_styles_buttons($colors = null) {
    /*buttons*/
    ...
    $style_style_6_button_bg_1 = get_option('style_style_6_button_bg_1', '#33a4f2');
    $style_style_6_button_bg_2 = get_option('style_style_6_button_bg_2', '#6ad5f9');
    $style_style_6_button_color = get_option('style_style_6_button_color', '#fff');
...
.btn.style-6,
.vc_btn.style-6   {
  background-color: <?php echo esc_attr($style_style_6_button_bg_1); ?>!important;
  background: linear-gradient(90deg, <?php echo esc_attr($style_style_6_button_bg_1); ?>, <?php echo esc_attr($style_style_6_button_bg_2); ?>) !important;
  color: <?php echo esc_attr($style_style_6_button_color); ?>!important;
  border: none;
}
<?php
}

add_action( 'wp_ajax_anps_styles', 'anps_custom_styles_ajax' );
add_action( 'wp_ajax_nopriv_anps_styles', 'anps_custom_styles_ajax' );

function anps_custom_styles_ajax() {
    $colors = $_POST['data'];

    ob_start();
    anps_custom_styles($colors);
    anps_custom_styles_buttons($colors);
