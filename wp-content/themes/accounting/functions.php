...
/* Allow different logo version for Polylang */
if (function_exists('pll_register_string')) {
    pll_register_string('anps_logo', get_option('anps_logo', ''));
    pll_register_string('anps_sticky_logo', get_option('anps_sticky_logo', ''));
    pll_register_string('anps_mobile_logo', get_option('anps_mobile_logo', ''));
}
...
function anps_scripts_and_styles() {
    ...
    wp_enqueue_style("font-awesome-4-5", get_template_directory_uri() . "/css/font-awesome.min.css");
    ...
    wp_enqueue_style("accounting", get_template_directory_uri() . "/css/accounting$rtl_suffix.css");
                ...
                <ul class="myaccount-menu">
                    <li class="widget-container widget_nav_menu">
                        <div class="menu-main-menu-container">
                            <ul class="menu">
                                <li class="menu-item<?php if($page == "myaccount"){ echo " current-menu-item page_item current_page_item current_page_parent"; } ?>"><a href="<?php echo get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ); ?>"><?php _e("My Orders", 'accounting'); ?></a></li>
                                <?php if ( in_array( 'yith-woocommerce-wishlist/init.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ): ?>
                                    <li class="menu-item<?php if($page == "wishlist"){ echo " current-menu-item page_item current_page_item current_page_parent"; } ?>"><a href="<?php echo get_permalink( get_option( 'yith_wcwl_wishlist_page_id' ) ); ?>"><?php _e("My Wishlist", 'accounting'); ?></a></li>
                                <?php endif; ?>
                                <li class="menu-item<?php if($page == "billing"){ echo " current-menu-item page_item current_page_item current_page_parent"; } ?>"><a href="<?php echo wc_get_endpoint_url( 'edit-address', 'billing' ); ?>"><?php _e("Edit Billing Address", 'accounting'); ?></a></li>
                                <li class="menu-item<?php if($page == "shipping"){ echo " current-menu-item page_item current_page_item current_page_parent"; } ?>"><a href="<?php echo wc_get_endpoint_url( 'edit-address', 'shipping' ); ?>"><?php _e("Edit Shipping Address", 'accounting'); ?></a></li>
                                <li class="menu-item<?php if($page == "edit_account"){ echo " current-menu-item page_item current_page_item current_page_parent"; } ?>"><a href="<?php echo wc_customer_edit_account_url(); ?>"><?php _e("Edit Account", 'accounting'); ?></a></li>
                                <?php
                                    if (is_user_logged_in()) {
                                        echo '<li><a href="'. wp_logout_url( get_permalink( woocommerce_get_page_id( 'myaccount' ) ) ) .'">' . __("Logout", 'accounting') . '</a></li>';
                                    }
                                ?>
                            </ul>
                        </div>
...
function anps_theme_styles() {
    ...
    if (get_option('font_source_1', "Google fonts")=='Google fonts') {
        ...
        wp_enqueue_style( "font_type_1",  'https://fonts.googleapis.com/css?family=' . get_option('font_type_1', 'PT+Serif') . ':400italic,400,500,600,700,300&subset='.$font1_implode_subset);
    } else {
        wp_enqueue_style( "font_type_1",  'https://fonts.googleapis.com/css?family=Open+Sans:400italic,400,500,600,700,300&subset=latin,latin-ext');
    }
    ...
    if (get_option('font_source_navigation', "Google fonts")=='Google fonts' && get_option('font_type_1', 'PT+Serif')!=get_option('font_type_navigation', 'PT+Serif')) {
        ...
        wp_enqueue_style( "font_type_navigation",  'https://fonts.googleapis.com/css?family=' . get_option('font_type_navigation', 'PT+Serif') . ':400italic,400,500,600,700,300&subset='.$font3_implode_subset);
    }

    if (get_option('anps_text_logo_source_1', "Google fonts")=='Google fonts' && get_option('font_type_1', 'PT+Serif')!=get_option('anps_text_logo_font', 'PT+Serif') && get_option('anps_text_logo_font', '') != '') {
    ...
    wp_enqueue_style( "theme_core_style", get_template_directory_uri() . "/css/core$rtl_suffix.css" );
...
add_action( 'wp_enqueue_scripts', 'anps_theme_styles' );
